...
def test_remove_Bike():
    assert (remove_stock('Bike', {'Bike':{'stock':5}}) == 
            {'Bike': {'stock': 4}})

def test_remove_Scooter():
    assert (remove_stock('Scooter', {'Scooter':{'stock':5}}) == 
            {'Scooter': {'stock': 4}})
...
def test_add_atv():
    assert (add_stock('Atv', {'Atv':{'price': 30, 'stock': 5}}) ==
            {'Atv': {'price': 30, 'stock': 6}})

def test_add_sbs():
    assert (add_stock('Side-by-Side', {'Side-by-Side':{'price': 30, 'stock': 5}}) ==
            {'Side-by-Side': {'price': 30, 'stock': 6}})

def test_add_Scooter():
    assert (add_stock('Scooter', {'Scooter':{'price': 30, 'stock': 5}}) ==
            {'Scooter': {'price': 30, 'stock': 6}})

def test_add_Bike():
    assert (add_stock('Bike', {'Bike':{'price': 30, 'stock': 5}}) ==
            {'Bike': {'price': 30, 'stock': 6}})
